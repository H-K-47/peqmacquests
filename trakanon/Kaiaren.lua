...
function event_trade(e)
	...
	if((item_lib.check_turn_in(e.self, e.trade, {item1 = 1683})) and (e.other:GetLevel()>=46) and (" .. e.other:Class() .. "=="Monk")) then --Celestial Fists
		e.self:Emote("eyes open wide and he attacks you!");
		e.other:QuestReward(e.self,0,0,0,0,1683); --Celestial Fists
		eq.attack(e.other:GetName());
		eq.spawn2(95040,0,0,2470,306,-339,0); --Kaiaren (True)
	end
	if((item_lib.check_turn_in(e.self, e.trade, {item1 = 1684})) and (e.other:GetLevel()>=46) and (" .. e.other:Class() .. "=="Monk")) then --Charred Scale
	...
	if((item_lib.check_turn_in(e.self, e.trade, {item1 = 1688, item2 = 1689})) and (e.other:GetLevel()>=46) and (" .. e.other:Class() .. "=="Monk")) then --Demon Fangs and Book of Celestial Fists
